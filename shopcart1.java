 ...
 * @author ELCOT
 */
@WebServlet(urlPatterns = {"/shopcart1"}, initParams = {
    @WebInitParam(name = "aaa", value = "123")
    , @WebInitParam(name = "bbb", value = "456")
    , @WebInitParam(name = "ccc", value = "789")})
public class shopcart1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
	    ...
	    String ccard=request.getParameter("c");	
            ...
            String pswd[]=config.getInitParameter("password").split(",");
            
            Cookie c;
             for(int i=0;i<user.length;i++)
            {
                
                if(name.equals(user[i]) && ccard.equals(card[i]) && password.equals(pswd[i]))
                {
